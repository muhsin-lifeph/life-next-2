...
const DynamicGrid: FC<props> = ({ data, isDesktop, isMobile }) => {
    ...
    return <div
        style={{ gridTemplateColumns: `${isDesktop && data.settings.desktop.column > 1? `repeat(${data.settings.desktop.column},auto)`:isMobile && data.settings.mobile.column > 1?`repeat(${data.settings.mobile.column},auto)`:""}`, gridTemplateRows:`${data.settings.mobile.row > 1?``:""}` }} className="grid" >
