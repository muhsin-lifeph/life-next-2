  ...
  return (
    <>
        ...
        <div className="  justify-between py-1 max-w-[1450px] mx-auto  sm:px-[10px] px-[5px] text-white lg:flex md:flex hidden " >
          <Link href={"/super-summer-savers"} className={"flex justify-start space-x-3 "}>
            <div className={` my-auto`}>
              <span className=" sm:text-sm  text-[10px]  whitespace-nowrap">{t.navbar.highest_rated_phar} <span className=" ml-2">|</span> </span>
            </div>
            <Image src={"https://www.lifepharmacy.com/images/app-rating.svg"} className="lg:w-20 w-14 h-5 my-auto" height={30} width={30} alt={"app-rating"} />
            <div className="flex items-center">
              <span className="font-semibold mr-2">|</span>
              <small className="text-sm whitespace-nowrap">Download Now</small>
            </div>
          </Link>
          <div className="text-end flex justify-between items-center ">
            <small className="mx-4 text-xs  overflow-hidden line-clamp-1" >{t.navbar.deliver_to}  {session?.token?.addresses && session?.token?.addresses.length != 0 ? (displayedAddress(AddressDataIndex)) : "Dubai, United Arab Emirates"}</small>
            <button
      ...
      <div className="sm:visible md:hidden ">
        <div className="flex  bg-life text-white text-[10px] px-[10px] py-1 justify-between items-center">
          <div> <b>{t.navbar.deliver_to}:</b>   <span className="mx-2">Business Bay, Dubai</span>  </div>
          <button className="bg-white rounded text-pink-700 py-0.5 px-2" onClick={() => { locationOnClickHandle() }}>CHANGE</button>
        </div>
      </div>
    </>
  )
