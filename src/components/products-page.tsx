...
export const ProductsPage = ({ filterPath, isSearchPage, categoryData, menuData, selectedBrands, type, isBrandsPage }: { categoryData: any, menuData: any, filterPath: string, isSearchPage: boolean, selectedBrands: string, type: string, isBrandsPage: boolean }) => {
    ...
    return (
        <div className="max-w-[1450px] mx-auto  sm:px-[10px] px-[5px]">
            {
                (categoryData.filters && categoryData.filters.categories && categoryData.filters.categories[0] && categoryData.filters.categories[0].images && categoryData.filters.categories[0].images.banner) || (isBrandsPage && categoryData.brand_details.images.banner)?
                    <div className=''>
                        {isBrandsPage ?
                        ...
                        {isBrandsPage ?
                            ...
                            <div className="relative">
                                <p className={`text-sm text-gray-600 my-5 ${readMoreClick ? '' : 'overflow-y-hidden h-[7rem]'} text-ellipsis leading-7`} dangerouslySetInnerHTML={{ __html: categoryData.brand_details.short_description }} />
            ...
            <ProductsPageData filterPath={filterPath} categoryData={categoryData} brandsData={categoryData.brands} isSearchPage={isSearchPage} selectedBrands={menuData[0] != "Category" ? selectedBrands : query.brands ? query.brands : ""} menuData={menuData} isBrandsPage={isBrandsPage} />
        </div>
    )
}